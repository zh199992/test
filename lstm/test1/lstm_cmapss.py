...
#region 第一种训练方法，用50*142的输出来计算loss
##################
lstm1 = LSTM(25, 16, output_size=1, num_layers=1).to(device)
print(lstm1)##########

optimizer = torch.optim.Adam(lstm1.parameters(), lr=0.01)   # optimize all cnn parameters
...
for epoch in range(100):
    ...
    b_x = seq_tensor[0:142].transpose(0,1)
    b_y = label_tensor[0:142]
    # b_x=b_x.squeeze()
    output1,_ = lstm1(b_x)               # cnn output
...
outputplt2 = output2.cpu().detach().numpy()
plt.plot(outputplt2,label='lstm2')
label_array = label_tensor[0:192-50].cpu().detach().numpy()
plt.plot(label_array,label='groundtruth')
plt.legend()
plt.show()
